...
L'objectif principale du pipeline NGS\_RG\_MGI est de générer et distribuer les fichires de séquences dans le bon répertoire de projet, d'échantillon, de type de séquençage et de run.
...
Les tableaux et graphiques de ces rapports de séquençage permettent de facilité l'évaluation du run et des ses pistes.
...
On y insère également certains métriques du run dont le readset fait partie, comme le nombre de cycles des reads et des index, la date de run, ect.
...
\subsection{Le pipeline de contôle qualité des fichiers de séquences pour la technologie MGI}
